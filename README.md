...
## 4-1. 자동 업데이트 (v0.4.1부터 실제 동작)

앱에 **자동 업데이트 기능**이 들어있다. 앱을 켜면 GitHub Releases의
...
| 이름 | 값 |
|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | 서버의 `~/.tauri-keys/tinadmin.key` 내용 (**base64 한 줄**, `=` 끝까지 전부) |
| `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` | **아예 만들지 않거나 빈 값**. 아무 문자나 넣으면 서명 실패 |
...
> ℹ️ **키 파일 형식 주의**: 이 파일은 평문이 아니라 **base64로 인코딩된 한 줄**이다.
> 열어보면 `dW50cnVzdGVkIGNvbW1lbnQ6...` 처럼 보이는 게 정상이다.
> (디코딩하면 `untrusted comment: rsign encrypted secret key` 로 시작한다 —
> minisign 이 아니라 **rsign** 이다.)
> 공개키 파일 `.pub` 도 같은 형식이라 겉보기로는 구분이 안 되니, 파일명으로 구분할 것.
...
### 자동 업데이트가 뜨는지 확인하는 법
1. 앱을 PC에 설치해 둔다 (updater가 실제로 도는 건 **v0.4.1 이상**)
2. 코드를 고치고 버전을 올려 태그를 push (예: v0.6.0)
3. 빌드가 끝나면 Releases에 `latest.json` + `.sig` 가 함께 올라간다
4. **설치해 둔 앱을 완전히 껐다가 다시 켜면** 왼쪽 아래에 업데이트 알림이 뜬다

### 빌드 실패 시 원인 확인
워크플로우에 `Pre-build (로그 캡처용)` 단계가 있어서, 빌드가 깨지면 에러 마지막 40줄이
GitHub **annotation** 으로 남는다. annotation 은 리포 admin 권한 없이도 API로 읽을 수 있다:
```bash
curl -s "https://api.github.com/repos/bogon0130/tinadmin-desktop/check-runs/<JOB_ID>/annotations"
```
> ⚠️ 이 Pre-build 단계는 진단용이라 **빌드를 두 번** 한다(느림).
> 안정화되면 이 단계를 지우는 게 좋다.
