...
import { loadQuickRaw } from "@/lib/quickcmds"
...
export function QuickFavoritesPanel() {
  ...
  const load = useCallback(async () => {
    ...
    // 예전 "명령 즐겨찾기"(quick_commands.json)를 한 번만 옮겨온다
    if (!migratedRef.current) {
      migratedRef.current = true
      try {
        const raw = await loadQuickRaw()
        if (raw.trim()) {
          const { store: merged, added } = migrateQuick(cur, raw)
          if (added > 0) {
            cur = merged
            await saveFavorites(cur)
            toast.success(`예전 명령 즐겨찾기 ${added}개를 옮겨왔습니다`)
          }
        }
      } catch {
        /* 예전 파일이 없거나 못 읽어도 그냥 넘어간다 */
      }
    }
    setStore(cur)
  }, [])
